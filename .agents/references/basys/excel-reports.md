...
Source: Converted from legacy rule `excel-reports.mdc`.

Codex usage: Conventions for creating and editing Excel report (Отчёт Excel) metaobjects in BaSYS — reports rendered by filling a pre-designed .xlsx template with values from data sources. Consult whenever the user asks to create, modify or debug an Excel report (отчёт Excel, печатная форма, регламентированная отчётность по макету), to add data sources / filters to an Excel report, or when editing files under `excel_report/`.

Applies to: `excel_report/**/*`.

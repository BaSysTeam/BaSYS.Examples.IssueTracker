...
Source: Converted from legacy rule `constructor-forms.mdc`.

Codex usage: Conventions for creating and editing constructor forms (формы-конструкторы) on metaobjects in BaSYS — a JSON tree of `FormElement` nodes authored visually in the form designer, stored as `*.form.*.json` next to the metaobject. Consult whenever the user asks to create, modify or debug a constructor form, list / edit form built in the visual designer, add a button / field / table / tab / collapsible group / toolbar / details table into such a form, or when editing files matching `*.form.*.json`. Covers wrapper hierarchy (`bs-form` → `bs-row` → `bs-col`), binding syntax (`$h.`, `$t.`, `$r.`, `vModel`, `:prop`, `v-bind`, `v-if`, `@Event`, `slot`), standard commands and the closed registry of constructor components.

Applies to: `**/*.form.*.json`.

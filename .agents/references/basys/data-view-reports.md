...
Source: Converted from legacy rule `data-view-reports.mdc`.

Codex usage: Conventions for building reports, dashboards, charts, tables, KPIs, gauges and pivot tables in BaSYS using the data_view (Панель данных) metaobject kind. Consult whenever the user asks for any kind of report, dashboard or visualization, or when editing files under data_view/.

Applies to: `data_view/**/*,**/*.bjs`.

...
Source: Converted from legacy rule `menu.mdc`.

Codex usage: Conventions for creating and editing menu (Меню) metaobjects in BaSYS — navigation menus rendered in the application's side panel via the PrimeVue MegaMenu component. Consult whenever the user asks to create, modify or debug a menu (меню, навигация, боковое меню, sidebar), to add a link / group / column / separator to a menu, to set up auto-fill of a menu group, or when editing files under `menu/`.

...
Source: Converted from legacy rule `print-forms.mdc`.

Codex usage: Conventions for creating and editing print forms (печатные формы) on metaobjects in BaSYS — per-instance documents rendered by filling a pre-designed .xlsx template with data of a specific object. Consult whenever the user asks to create, modify or debug a print form (печатная форма, печать документа, печатная карточка, счёт/накладная/акт по образцу для конкретного документа), or when editing files matching `*.print_form.*`.

Applies to: `**/*.print_form.*`.

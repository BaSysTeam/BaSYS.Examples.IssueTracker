...
Полные правила — в правиле `records-creation` (`.agents/references/basys/records-creation.md`). Краткий минимум для этого навыка:
...
5. Замени `$schema` на `"../../system/schemas/metaObjectStorableSettings.schema.json"` (в файлах примеров используется более глубокий относительный путь, потому что они лежат под `.agents/skills/create-operation/examples/`).
